...
- 🎯 Entry points to execute your program using `python -m <sa>` or `$ sa` with basic CLI argument parsing.
...
---
# sa Flask Application

Awesome sa created by Amlakkolbe20
...
```bash
git clone https://github.com/Amlakkolbe20/Sa sa
cd sa
make install
```
...
```bash
sa create-db   # run once
sa populate-db  # run once (optional)
sa add-user -u admin -p 1234  # ads a user
sa run
```
